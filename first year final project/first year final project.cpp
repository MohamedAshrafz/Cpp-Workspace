...
matrix_F[N][N], matrix_Fac[N][N], matrix_G[N][N], matrix_H[N][N], matrix_J[N][N];
static float temp[N][N];
int rows1, columns1, rows2, columns2;
string op;
void input_matrix(float[N][N], int, int);
...
int main()
{
	...
	cout << endl << "fill A matrix as (row, column)" << endl;

	input_matrix(matrix_A, rows1, columns1);
	...
	cout << endl << "fill B matrix as (row, col)  " << endl;

	input_matrix(matrix_B, rows2, columns2);
	...
	while (true)
	{
		...
		else if (op == "9")
		{
			...
			view_matrix(matrix_F, columns2, rows2);
...
void input_matrix(float arr[N][N], int rows, int columns)
{
